...
pub const PATH_NOTIFICATION: &str = "/api/basicserver/agency/notification/pn/pending/read/count";
...
pub const PATH_BOOKING_OVERVIEW: &str = "/api/line_haul/agency/booking/bidding/booking_overview";
...
impl SpxClient {
    ...
    pub async fn fetch_bookings(
        ...
        pageno: u32,
        count: u32,
    ) -> Result<Vec<SpxBooking>, SpxError> {
        ...
        Ok(extract_booking_list(&json)
            .iter()
            .map(normalize_booking)
            .collect())
    }
    ...
    pub async fn fetch_request_list(
        ...
        self.post_json(
            cookies,
            PATH_REQUEST_LIST,
            json!({ "booking_id": booking_id, "pageno": 1, "count": count }),
        )
        .await
    }
    ...
    pub async fn accept_booking(
        ...
        if agency_id <= 0 {
            return AcceptResult {
                success: false,
                reason: AcceptReason::Auth,
                retcode: -1,
                message: "agency_id kosong".into(),
            };
        }
    ...
    pub async fn notification_count(&self, cookies: &SpxCookies) -> Result<Value, SpxError> {
        self.post_json(
            cookies,
            PATH_NOTIFICATION,
            json!({ "use_case": "agency portal", "user_type": 4, "notification_type_list": [30] }),
        )
        .await
    }

    /// bidding log/list (GET) — the acceptor op-log.
    pub async fn fetch_bidding_log(
        &self,
        cookies: &SpxCookies,
        booking_id: i64,
    ) -> Result<Value, SpxError> {
    ...
    pub async fn fetch_agency_users(
        &self,
        cookies: &SpxCookies,
        agency_id: i64,
    ) -> Result<Value, SpxError> {
        self.post_json(
            cookies,
            PATH_USER_LIST,
            json!({ "request_source": 1, "agency_id": agency_id, "pageno": 1, "count": 100 }),
        )
        .await
    }
    ...
    pub async fn fetch_booking_log(
        &self,
        cookies: &SpxCookies,
        booking_id: i64,
    ) -> Result<Value, SpxError> {
        self.post_json(
            cookies,
            PATH_BOOKING_LOG,
            json!({ "booking_id": booking_id, "pageno": 1, "count": 20 }),
        )
        .await
    }
}
...
mod extract_tests {
    ...
    fn extract_prefers_list_then_booking_list() {
        ...
        let b =
            json!({ "data": { "booking_list": [{ "booking_id": "1" }, { "booking_id": "2" }] } });
