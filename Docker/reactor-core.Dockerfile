...
# transitively by spx-client's wreq dependency, Fase 3 Task 8). reactor-core
# does not depend on spx-client yet, so this is not load-bearing today, but
# once poller/executor (Fase 4-5) wire spx-client into this binary's build
...
# into reactor-core, or TLS verification fails closed with no trust anchors.
