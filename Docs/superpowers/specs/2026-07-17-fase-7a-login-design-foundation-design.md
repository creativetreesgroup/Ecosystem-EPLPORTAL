...
## Tracked note: Origin allowlist untuk `/ws` sebagai defense-in-depth (Task 4 whole-branch review finding — non-blocking, dinilai TIDAK exploitable hari ini)

Scanner keamanan otomatis menandai `ws_handler_with_auth`'s alur cookie baru (Task 4) sebagai potensi Cross-Site WebSocket Hijacking (CSWSH) karena tidak ada validasi header `Origin` sebelum menerima upgrade WS. Dinilai (opus review, dengan analisis independen terhadap kode `auth.rs` yang sebenarnya) **TIDAK exploitable hari ini**: cookie sesi (`spx_session`) di-set dengan `SameSite::Strict` (`auth.rs::portal_login`) — browser yang correctly mengimplementasikan SameSite TIDAK PERNAH melampirkan cookie ini ke request cross-site (termasuk WS handshake, yang merupakan request HTTP biasa sebelum upgrade), jadi request dari origin lain akan tiba tanpa cookie sama sekali dan ditolak 401 sebelum upgrade — tidak ada hijack yang mungkin. Gap residual yang TIDAK ditutup `SameSite=Strict` (dienforce per registrable-domain, bukan per-origin-eksak): subdomain sibling di domain yang sama akan dianggap same-site dan cookie tetap terlampir — ancaman jauh lebih sempit (butuh pijakan attacker di domain terdaftar yang sama), bukan skenario cross-site yang dideskripsikan scanner. `AppState.cors_origins: Arc<Vec<String>>` sudah ada dan bisa dipakai sebagai allowlist `Origin` di `/ws` (pola thread-satu-parameter-lagi yang sama seperti `cookie_name` di Task 4) sebagai defense-in-depth tambahan menutup gap residual itu — dilacak sebagai follow-up, bukan blocker merge Fase 7a.
