...
**`BookingListItem`** (`api-gateway/src/routes/bookings.rs`) gains: `request_id`, `onsite_id`, `booking_number` (from `spx_tx_id`), `vehicle_type`, `deadline_at`, `pickup_time`, `trip_type`, `booking_type` (`"coc"` | `"reguler"`, from the existing `is_coc`-equivalent logic — reuse `booking_type_of`/`Spxid` mapping already in `core_domain`).

**Superseded during implementation (whole-branch review, ratified — not reverted):** this section originally specified deriving all of the above via `spx_client::normalize_booking(&b.raw_data)` at read time, matching how `route` already works, so that generated columns stayed filter/sort-only and never became a second source of truth for display. The shipped implementation instead sources these fields from the same `spx_*` generated columns (migration 0021) used for filtering — a deliberate deviation, confirmed better on review: it makes filter and display provably consistent (a row filtered as vehicle X is guaranteed to display as vehicle X, since both reads are now the same read), rather than relying on two independently-written derivations staying in sync by convention. `route` remains the one exception, still sourced from `normalize_booking` (predates this sub-phase, not itself SQL-filterable). If `BookingDetail` is ever expanded to carry these fields (see Task 7's disclosed gap below), it must source them from the same generated columns, not `normalize_booking` — see the code comment on `BookingListItem` in `api-gateway/src/routes/bookings.rs`.
